...
const AddVoucher = (props) => {
  ...
  let handleSaveInforVoucher = async () => {
    ...
    if (!inputValues.codeVoucher) {
      toast.error("Vui lòng nhập tên mã voucher.");
      return;
    }

    // Kiểm tra ngày bắt đầu và kết thúc
    const startDate = new Date(inputValues.fromDate);
    const endDate = new Date(inputValues.toDate);
    if (startDate >= endDate) {
      toast.error("Ngày bắt đầu phải nhỏ hơn ngày kết thúc.");
      return;
    }

    // Kiểm tra giá trị số lượng mã (amount)
    const amountValue = parseInt(inputValues.amount, 10);
    if (amountValue <= 0) {
      toast.error("Số lượng mã phải lớn hơn 0.");
      return;
    }
    if (amountValue > 5000000) {
      toast.error("Số lượng mã không được vượt quá 5.000.000 VND.");
      return;
    }
